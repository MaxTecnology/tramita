# Usuários — Melhoria Completa — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reescrever `Users.tsx` com paridade de qualidade em relação a `Clients.tsx` — edição via modal, phone no formulário, validação, erros inline, confirmação no delete, busca por texto e filtro por perfil.

**Architecture:** Reescrita completa de um único arquivo frontend. Zero mudanças no backend — todos os endpoints já existem (`GET/POST/PATCH/DELETE /users`). Filtros client-side via `useMemo`, estilo alinhado com `Clients.tsx` (divs diretos, sem `Card`/`CardContent`).

**Tech Stack:** React 19, TanStack Query v5, TailwindCSS v4, shadcn/ui (`Dialog`), TypeScript strict

---

## Mapa de Arquivos

- Modify: `apps/web/src/pages/app/Users.tsx` — reescrita completa (único arquivo)
- `apps/web/src/types/index.ts` — sem mudança (interface `User` já está correta)

---

## Task 1: Reescrever `Users.tsx`

**Files:**
- Modify: `apps/web/src/pages/app/Users.tsx`

Não há testes de frontend a escrever — lógica puramente de UI state + mutations TanStack Query sem lógica de negócio crítica.

- [ ] **Substituir o conteúdo completo de `apps/web/src/pages/app/Users.tsx` por:**
...
git commit -m "feat: Users.tsx — edição modal, busca, filtro por perfil e paridade com Clients"
```
...
- [ ] `useMemo` e `useState` importados
- [ ] Estados: `showCreate`, `createForm` (com phone), `editingUser`, `editForm`, `deletingId`, `search`, `roleFilter`
- [ ] `createMutation` com guards (`!name || !email || !password`), phone opcional, reset de filtros no `onSuccess`
- [ ] `updateMutation` recebe `EditForm & { id: string }` — sem non-null assertion
- [ ] `deleteMutation` com `deletingId`, `window.confirm`, loading/disabled por item, `setDeletingId(null)` no `onSuccess`
- [ ] `filtered` via `useMemo` (search em name+email, roleFilter)
- [ ] Barra de filtros: input com `aria-label` + toggle Todos/Gerente/Colaborador
- [ ] Contador condicional (total ou "Exibindo X de Y")
- [ ] Cards com divs diretos (sem `Card`/`CardContent`), badge de perfil colorido, phone condicional
- [ ] Botões Editar/Desativar ocultos para `ORG_ADMIN`
- [ ] Modal de edição com `Dialog` shadcn (nome, e-mail, perfil, phone)
- [ ] Formulário de criação com campo phone
- [ ] Erros inline em criação (`createMutation.isError`) e edição (`updateMutation.isError`)
- [ ] Header toggle reseta `createForm` ao fechar
