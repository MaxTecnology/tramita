...
export default function PortalBoards() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-4">
      <h1 className="text-lg md:text-xl font-bold text-foreground">Meus Processos</h1>
          ...
          {boards.map((board) => {
            ...
            return (
              <Link
                ...
                className="block rounded-xl border border-border bg-surface p-4 shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  <h2 className="text-sm font-semibold text-foreground leading-snug flex-1 min-w-0">{board.title}</h2>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {board.dueDate && (
                    <span className={cn('text-xs ml-auto flex-shrink-0', isOverdue ? 'text-red-500 font-medium' : 'text-muted-foreground')}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground w-8 text-right flex-shrink-0">{progress}%</span>
                </div>
              </Link>
            )
          })}
